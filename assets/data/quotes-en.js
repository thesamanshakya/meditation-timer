export const quotes = [
  {
    "quote": "Inward calm cannot be maintained unless physical strength is constantly and intelligently replenished.",
    "author": "The Buddha"
  },
  {
    "quote": "Learn to be calm and you will always be happy.",
    "author": "Paramahansa Yogananda"
  },
  {
    "quote": "You will always be getting praise and blame, but do not let either affect the poise of the mind.",
    "author": "The Buddha"
  },
  {
    "quote": "Do not dwell in the past, do not dream of the future, concentrate the mind on the present moment.",
    "author": "The Buddha"
  },
  {
    "quote": "Feelings come and go like clouds in a windy sky. Conscious breathing is my anchor.",
    "author": "Thich Nhat Hahn"
  },
  {
    "quote": "To understand the immeasurable, the mind must be extraordinarily quiet, still.",
    "author": "Jiddu Krishnamurti"
  },
  {
    "quote": "Meditation is like a gym in which you develop the powerful mental muscles of calm and insight.",
    "author": "Ajahn Brahm"
  },
  {
    "quote": "The act of meditation is being spacious.",
    "author": "Sogyal Rinpoche"
  },
  {
    "quote": "We must experience the Truth in a direct, practical and real way. This is only possible in the stillness and silence of the mind; and this is achieved by means of meditation.",
    "author": "Samael Aun Weor"
  },
  {
    "quote": "If the mind falls asleep, awaken it. Then if it starts wandering, make it quiet. If you reach the state where there is neither sleep nor movement of mind, stay still in that, the natural (real) state.",
    "author": "Ramana Maharshi"
  },
  {
    "quote": "The more regularly and the more deeply you meditate, the sooner you will find yourself acting always from a center of peace.",
    "author": "J. Donald Walters"
  },
  {
    "quote": "The trouble is, you think you have time.",
    "author": "The Buddha"
  },
  {
    "quote": "Believe nothing, no matter where you read it, or who said it, no matter if I have said it, unless it agrees with your own reason and your own common sense.",
    "author": "The Buddha"
  },
  {
    "quote": "No one saves us but ourselves. No one can and no one may. We ourselves must walk the path.",
    "author": "The Buddha"
  },
  {
    "quote": "The root of suffering is attachment.",
    "author": "The Buddha"
  },
  {
    "quote": "Your goal is not to battle with the mind, but to witness the mind.",
    "author": "Swami Muktananda"
  },
  {
    "quote": "Learn this from water: loud splashes the brook but the oceans depth are calm.",
    "author": "The Buddha"
  },
  {
    "quote": "You only lose what you cling to.",
    "author": "The Buddha"
  },
  {
    "quote": "The past is already gone, the future is not yet here. There's only one moment for you to live.",
    "author": "The Buddha"
  },
  {
    "quote": "Holding onto anger is like drinking poison and expecting the other person to die.",
    "author": "The Buddha"
  },
  {
    "quote": "What you think, you become. What you feel, you attract. What you imagine, you create.",
    "author": "The Buddha"
  },
  {
    "quote": "If you are quiet enough, you will hear the flow of the universe. You will feel its rhythm. Go with this flow. Happiness lies ahead. Meditation is key.",
    "author": "The Buddha"
  },
  {
    "quote": "To conquer oneself is a greater task than conquering others.",
    "author": "The Buddha"
  },
  {
    "quote": "Purity or impurity depends on oneself. No one can purify another.",
    "author": "The Buddha"
  },
  {
    "quote": "Work out your own salvation. Do not depend on others.",
    "author": "The Buddha"
  },
  {
    "quote": "Your worst enemy cannot harm you as much as your own unguarded thoughts.",
    "author": "The Buddha"
  },
  {
    "quote": "You will not be punished for your anger, you will be punished by your anger.",
    "author": "The Buddha"
  },
  {
    "quote": "Nothing is permanent.",
    "author": "The Buddha"
  },
  {
    "quote": "Thousands of candles can be lit from a single candle, and the life of the candle will not be shortened. Happiness never decreases by being shared.",
    "author": "The Buddha"
  },
  {
    "quote": "There is no path to happiness. Happiness is the path.",
    "author": "The Buddha"
  },
  {
    "quote": "Happiness does not depend on what you have or who you are. It solely relies on what you think.",
    "author": "The Buddha"
  },
  {
    "quote": "Happiness is not having a lot. Happiness is giving a lot.",
    "author": "The Buddha"
  },
  {
    "quote": "If you truly loved yourself, you could never hurt another.",
    "author": "The Buddha"
  }
]
